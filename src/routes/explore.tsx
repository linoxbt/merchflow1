...
function Explore() {
  ...
  const protocol = stats.data?.stats ?? {
    invoicesPaid: 0,
    totalVolumeQie: 0,
    activeMerchants: 0,
    payrollProcessedQie: 0,
  };
    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 py-8 space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Live stats across every merchant on MerchFlow.
        </p>
      </div>
              ...
              {activity.map((e) => {
                ...
                return (
                  <li key={e.id} className="flex items-center gap-3 py-3">
                    <span
                      className={`inline-flex items-center gap-1.5 rounded px-2 py-0.5 text-[10px] font-mono uppercase border ${meta.cls}`}
                    >
                      <Icon className="h-3 w-3" />
                      {meta.label}
                    </span>
                    <span className="font-mono text-xs text-muted-foreground">
                      {truncateAddress(e.actor_wallet)}
                    </span>
            ...
            <ul className="space-y-3">
              {merchants.map((m, i) => (
                <li key={m.wallet} className="flex items-center gap-3">
                  <span className="h-6 w-6 rounded-md bg-surface-2 grid place-items-center text-xs font-mono">
                    {i + 1}
                  </span>
