  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 py-8 space-y-6">
              ...
              <tbody>
                {filtered.map((inv) => (
                  ...
                  >
                    ...
                    <td className="py-3 px-4 text-muted-foreground max-w-[260px] truncate">
                      {inv.description}
                    </td>
                    <td className="py-3 px-4 text-right">
                      <div className="font-mono">{formatQie(num(inv.amount_qie))} QIE</div>
                      <div className="font-mono text-[10px] text-muted-foreground">
                        ${formatQie(num(inv.amount_usd))}
                      </div>
                    </td>
      ...
      <QrScanner
        ...
        onScan={(text) => {
          window.location.href = getPaymentUrl(parsePaymentTarget(text), window.location.origin);
        }}
      />
    </div>
  );
  ...
  return (
    <div className="py-16 text-center">
      ...
      <div className="font-medium">
        {hasAny ? "No invoices match this filter" : "No invoices yet"}
      </div>
      <div className="text-sm text-muted-foreground mt-1">
        {hasAny
          ? "Try clearing the filter or search."
          : "Create your first invoice to start building your payment history."}
      </div>
      {!hasAny && (
...
function CreateInvoiceDialog({
  ...
  const amountQie = useMemo(() => {
    ...
    return Number.isFinite(n) ? n * QIE_ACCOUNTING_RATE : 0;
  }, [amountUsd]);
  ...
  const submit = async () => {
    ...
    if (!/^0x[a-fA-F0-9]{40}$/.test(customer)) {
        ...
        {!created ? (
            ...
            <div className="grid gap-4 mt-2">
              ...
              <div>
                ...
                <Textarea
                  ...
                  placeholder="Web development services — July"
                  className="mt-1.5"
                />
              </div>
          ...
          <div className="text-center py-4">
            ...
            <div className="font-mono text-muted-foreground text-sm">
              {formatQie(num(created.amount_qie))} QIE
            </div>
            ...
            <div className="mt-4 flex justify-center gap-2">
              <Button
                ...
                className="border border-border"
                onClick={() => {
                  navigator.clipboard.writeText(getPaymentUrl(created.number));
            ...
            <div className="mt-6 space-y-5 text-sm">
              <div>
                ...
                <div className="font-mono mt-1">
                  {truncateAddress(invoice.customer_wallet, 10, 8)}
                </div>
              </div>
              <div>
                <div className="text-[10px] font-mono uppercase text-muted-foreground">
              ...
              <div>
                ...
                <div className="font-mono text-2xl mt-1">
                  {formatQie(num(invoice.amount_qie))}{" "}
                  <span className="text-sm text-muted-foreground">QIE</span>
                </div>
                <div className="font-mono text-xs text-muted-foreground">
                  ≈ ${formatQie(num(invoice.amount_usd))} USD
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
              ...
              <div className="rounded-md border border-border bg-background p-4 grid place-items-center">
                <QRCodeSVG
                  value={getPaymentUrl(invoice.number)}
                  size={150}
                  bgColor="transparent"
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  onClick={() => {
                    navigator.clipboard.writeText(getPaymentUrl(invoice.number));
                    toast.success("Payment link copied");
                  }}
                >
              ...
              {invoice.tx_hash && (
                <div>
                  ...
                  <a
                    className="font-mono text-xs text-primary inline-flex items-center gap-1 mt-1 hover:underline"
                    href="#"
                  >
...
function QrModal({ invoice, onClose }: { invoice: InvoiceRow | null; onClose: () => void }) {
  ...
  const payload = getPaymentUrl(invoice.number);
  const downloadSvg = () => {
